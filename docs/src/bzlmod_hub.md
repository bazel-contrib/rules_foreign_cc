...
ninja as prebuilt binaries, meson built from source, m4, make and pkgconfig
wrapped from the `m4` / `make` / `pkgconf` Bazel Central Registry modules rfcc
`bazel_dep`s on, plus autoconf, automake, and msbuild as system tools - and
**registers the hub for you** from its own MODULE.bazel. (For the exact
...
| `meson` | custom, source, system, noop | source | yes |
| `msbuild` | custom, system, noop | system | no |
| `ninja` | binary, custom, source, system, noop | binary | yes |
