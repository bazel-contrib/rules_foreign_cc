...
# to the path of the real file. Therefore, for it to find the dependant shared
# libs, we need the path to be in execroot coordinates. On darwin, @loader_path
# in LC_RPATH behave the same way. Hence converting `../` to `external/`.
...
def _origin_short_paths_from_install_tree(installdir_short_path, install_tree_origins):
    if install_tree_origins and installdir_short_path == None:
        fail(
            ...
            "declared output from out_shared_libs, out_binaries, " +
            "out_data_dirs, or out_data_files to determine the install directory.",
        )
...
def runtime_library_search_directories_enabled(ctx):
    """Returns true when runtime library search directory derivation is enabled.

    Args:
      ctx: Rule context.

    Returns:
      True when runtime library search directories should be derived.
    """
...
export_for_test = struct(
    installdir_short_path_from_outputs = _installdir_short_path_from_outputs,
