...
TOOL_SPECS = {
    ...
    "meson": struct(
        ...
        # default_versions_in_sync_test enforces it for every tool. Unusually,
        # this one version names two things that must stay equal: the `@meson`
        # registry module the default `custom` registration wraps, and the
        # `@meson_src_<version>` archive `mode = "source"` and the hub's
        # `meson_src_*` aliases still use.
...
# Tools whose binary comes from a registry module (@m4, @make, @meson, @ninja,
# @pkgconf) rather than from an archive rfcc builds. Only these specs have a
# non-None `bcr_binary` and `workspace_bcr_toolchain`.
#
# This is *not* the complement of SOURCE_TOOLS: make, meson, ninja and
# pkgconfig are in both -- they build from their own archive under
# `mode = "source"` and use the registry module under `mode = "custom"` --
# while m4 is registry-only. cmake is the one source tool with no module.
BCR_BACKED_TOOLS = [
    name
