...
def cc_external_rule_impl(ctx, attrs):
    ...
    if outputs.header_manifest != None:
        rule_outputs.append(outputs.header_manifest)
    cc_toolchain = find_cpp_toolchain(ctx)
    ...
    return [
        ...
        ForeignCcFacadeInputsInfo(
            ...
            header_manifest = outputs.header_manifest,
...
def _emit_header_manifest(outputs):
    if outputs.out_include_dir == "" or outputs.header_manifest == None:
        return []

    include_dir = "$$EXT_BUILD_ROOT$$/{}".format(outputs.out_include_dir.path)
    manifest = "$$EXT_BUILD_ROOT$$/{}".format(outputs.header_manifest.path)
    return [
        "header_root=\"{}\"".format(include_dir),
        "header_manifest=\"{}\"".format(manifest),
        "prefix=\"$header_root/\"",
        "mkdir -p \"$(dirname \"$header_manifest\")\"",
        "rm -f \"$header_manifest\"",
        "if [ -d \"$header_root\" ]; then",
        "  find \"$header_root\" \\( -type f -o -type l \\) | LC_ALL=C sort | while IFS= read -r header; do",
        "    rel=\"${header#$prefix}\"",
        "    printf '%s\\n' \"$rel\" >> \"$header_manifest\"",
        "  done",
        "else",
...
_Outputs = provider(
    ...
    fields = dict(
        ...
        header_manifest = "Manifest of relative header paths under out_include_dir",
...
def _define_outputs(ctx, attrs, lib_name):
    ...
    has_facade_headers = bool(static_libraries) or bool(attr_shared_libs) or bool(attr_interface_libs)

    if attrs.out_include_dir:
        out_include_dir = ctx.actions.declare_directory(lib_name + "/" + attrs.out_include_dir)
        header_manifest = ctx.actions.declare_file(lib_name + "/.foreign_cc_headers_manifest") if has_facade_headers else None
    else:
        out_include_dir = ""
