...
def _create_configure_script(configureParameters):
    ...
    configure = create_configure_script(
        ...
        configure_command = ctx.expand_location(ctx.attr.configure_command, targets=ctx.attr.tools),
        deps = ctx.attr.deps,
        inputs = inputs,
        ...
        autogen_command = ctx.expand_location(ctx.attr.autogen_command, targets=ctx.attr.tools),
