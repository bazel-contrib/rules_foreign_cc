...
load("//foreign_cc:providers.bzl", "ForeignCcDepsInfo", "ForeignCcRuntimeExecutableInfo")
...
def _assert_runtime_executable_contract(env, target_name):
    ...
    asserts.true(
        env,
        "test/runtime_executable_fake_with_dep/bin/tool" in runfile_short_paths,
        "selected binary should be in runfiles",
    )
    asserts.true(
        env,
        "test/runtime_executable_fake_with_dep/share/runtime_resource.txt" in runfile_short_paths,
        "declared output resources should be in runfiles",
    )
    asserts.true(
        env,
        "test/runtime_executable_fake_with_dep/lib/libtool.so" in runfile_short_paths,
        "shared libraries should be in runfiles",
    )
    asserts.true(
        env,
        "test/runtime_executable_fake_with_dep/include" in runfile_short_paths,
        "include directory should be in runfiles",
    )
    asserts.true(
        env,
        "test/runtime_executable_fake_dep/share/runtime_resource.txt" in runfile_short_paths,
        "transitive declared output resources should be in runfiles",
    )
    asserts.false(
        env,
        "test/runtime_executable_fake_with_dep/bin/tool-$USER" in runfile_short_paths,
        "unselected binaries should not be in runtime executable runfiles",
    )
    asserts.false(
        env,
        "test/runtime_executable_fake_with_dep/debug/tool" in runfile_short_paths,
        "unselected binaries should not be in runtime executable runfiles",
    )
    asserts.false(
        env,
        "test/runtime_executable_fake_with_dep/lib/libtool.a" in runfile_short_paths,
        "static libraries should not be in runtime executable runfiles",
    )
    asserts.false(
        env,
        "test/runtime_executable_fake_with_dep/lib/tool.ifso" in runfile_short_paths,
