...
def _rfcc_defaults():
    ...
    exclusion MODULE.bazel and PREINSTALLED_TOOLS encode). cmake and ninja get
    a second, system-mode fallback tag mirroring MODULE.bazel: their prebuilt
    binaries only cover five platforms, so the unconstrained system toolchain
    catches every other host. Derived from tool_specs so the test fixture
    can't drift from the specs it tests.
    """
    out = {}
