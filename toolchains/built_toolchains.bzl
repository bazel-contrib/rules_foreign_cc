...
# buildifier: disable=bzl-visibility
load("//toolchains/private:hub.bzl", "hub_repo", "meson_extra_aliases", "source_spoke_aliases")
load(
...
# is narrower than SOURCE_TOOLS (what bzlmod *can* build). cmake is the last
# one left -- every other source tool now has a registry module WORKSPACE uses
# instead, with `@<tool>_src_<version>` reduced to a compat shim over it.
_WORKSPACE_SOURCE_TOOLS = ["cmake"]
...
def built_toolchains(
    ...
    Args:
        ...
        meson_version: The meson version, used the same way as `make_version`.
...
def _emit_workspace_hub(versions_by_tool):
    ...
    # is wider: make, meson, ninja and pkgconfig build from source under bzlmod
    # but come from a registry module here.
    aliases = []
    ...
    # meson's two extras are the one part of the source-spoke alias set that
    # has to survive the move to the registry module: meson_with_requirements
    # names them through //foreign_cc, so they must resolve under both
    # dependency models. bzlmod still serves them from the source spoke, which
    # is the same meson at the same version -- `default_versions_in_sync_test`
    # is what keeps that true.
    if versions_by_tool["meson"]:
        aliases.extend(meson_extra_aliases("meson"))
