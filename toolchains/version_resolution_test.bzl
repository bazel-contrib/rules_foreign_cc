...
def _cmake_wildcard_keys_test_impl(ctx):
    ...
    asserts.true(env, "3.19.x" in CMAKE_SRC_SRCS, "expected 3.19.x wildcard key")
    asserts.true(env, "3.19.8" in CMAKE_SRC_SRCS, "expected 3.19.8 exact key")

    # Wildcard and exact-latest point at the same entry.
    asserts.equals(env, CMAKE_SRC_SRCS["3.19.8"], CMAKE_SRC_SRCS["3.19.x"])

    # A dropped non-latest patch is absent (hard-fail at lookup time).
    asserts.false(env, "3.19.4" in CMAKE_SRC_SRCS, "3.19.4 should be dropped")
...
def version_resolution_test_suite(name):
    unittest.suite(
        name,
        partial.make(cmake_wildcard_keys_test, size = "small"),
    )
