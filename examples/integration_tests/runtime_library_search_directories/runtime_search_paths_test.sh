...
app_runtime_path="$loader/../lib"
middle_runtime_path="$loader/."
solib_path="$loader/../../../../_solib_"
solib_sibling_path="$loader/../_"
...
# Loader-relative paths the target is expected to carry in addition to the
# baseline paths above, e.g. those derived from
# additional_*_runtime_library_search_origins. Asserted per output kind, because
# an executable origin only affects binaries and a dynamic origin only affects
# shared libraries: EXPECTED_ADDITIONAL_BINARY_RUNTIME_PATHS against runtime_app,
# EXPECTED_ADDITIONAL_LIBRARY_RUNTIME_PATHS against libmiddle. Each is checked
# for presence; the binary/library may carry other paths too.
assert_additional_paths() {
  ...
  for rel in $expected_paths; do
    for file in "$@"; do
      assert_path "$file" "$loader/$rel"
    done
  done
}

if [[ -n "${EXPECTED_ADDITIONAL_BINARY_RUNTIME_PATHS:-}" && "${#app_files[@]}" -gt 0 ]]; then
  assert_additional_paths "$EXPECTED_ADDITIONAL_BINARY_RUNTIME_PATHS" "${app_files[@]}"
fi

if [[ -n "${EXPECTED_ADDITIONAL_LIBRARY_RUNTIME_PATHS:-}" && "${#middle_files[@]}" -gt 0 ]]; then
  assert_additional_paths "$EXPECTED_ADDITIONAL_LIBRARY_RUNTIME_PATHS" "${middle_files[@]}"
fi
