...
.PHONY: all leaf middle app install-leaf install-middle install-app install-plugin clean test
...
# Installs a shared library into a data subdirectory of lib so a target can
# declare an out_data_dirs output alongside an additional dynamic origin that
# points into it.
install-plugin: leaf
	mkdir -p $(PREFIX)/lib/plugins
	cp -p $(LEAF_SHARED_LIBRARY_PATH) $(PREFIX)/lib/plugins/
