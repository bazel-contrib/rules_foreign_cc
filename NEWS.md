...
- **The bzlmod `tools` extension is now hub-and-spoke.** Register the single
  `@rules_foreign_cc_toolchains//:all` target instead of individual spoke
  targets. The version-suffixed source-archive repos (e.g. `@cmake_src_3.31.12`)
  replace the previous singleton names (`@cmake_src`); there is no zero-edit
  migration path. See
...
- **The public `@rules_foreign_cc//toolchains:built_*_toolchain` labels were
  ...
  The build-from-source toolchains are now per-version spokes registered through
  the hub. Consumers that registered these labels directly should declare the
  matching `tools.<tool>(mode = "source", version = "...")` tag (an explicit
  version is required), or register the spoke-provided toolchain. See
  [Migrating to the bzlmod hub](docs/src/bzlmod_migration.md)
  ([#1567](https://github.com/bazel-contrib/rules_foreign_cc/pull/1567)).
- **`make`, `ninja` and `pkg-config` are no longer bootstrapped from source by
  `rules_foreign_cc`.** Under both dependency models, source mode now uses
  their Bazel Central Registry modules (`@make//:make`, `@ninja//:ninja`,
  `@pkgconf//:pkg-config`), which carry maintained Bazel-native `BUILD` files,
  so build problems are fixed upstream where every consumer shares the fix. The
  `make_tool`, `ninja_tool` and `pkgconfig_tool` rules are kept and still load,
  but are no longer instantiated and will not be updated as those projects'
  build requirements move upstream
  ([#1582](https://github.com/bazel-contrib/rules_foreign_cc/pull/1582)).
  Consequences:
  - **pkg-config is now `pkgconf`**, the pkg-config implementation most
    distributions have shipped as `pkg-config` for years, and the default
    `pkgconfig` version moves from `0.29.2` to `3.0.7` accordingly. It is a
    different project with its own version series, so previously valid
    `pkgconfig_version` / `tools.pkgconfig(version = ...)` values no longer
    resolve. The binary is published as both `@pkgconf//:pkgconf` and
    `@pkgconf//:pkg-config`; rfcc's toolchain uses the latter, so anything
    searching `PATH` for `pkg-config` (configure scripts, CMake's
    `FindPkgConfig`) is unaffected.
  - The hub no longer publishes `<tool>_src_all` for these three; the source
    trees have no replacement, because the modules expose no target for them.
    Under `bzlmod` `<tool>_built` is gone as well -- use the module targets
    above. The `glib_dev`, `glib_src` and `gettext_runtime` archives the old
    from-source pkg-config needed on Windows are still declared under both
    models, but only so `pkgconfig_tool` keeps loading; rfcc never builds them.
  - Under `WORKSPACE` the old surface is kept as shims over the registry build.
    `rules_foreign_cc_dependencies(make_version = ..., ninja_version = ...,
    pkgconfig_version = ...)` still selects the version, `@make_src_<version>`
    / `@ninja_src_<version>` / `@pkgconfig_src_<version>` still carry
    `<tool>_toolchain`, `<tool>_tool` and `<tool>_built`, and the hub still
    publishes `<tool>_built`. Two differences: `<tool>_built` is now the
    module's `cc_binary`, so `$(execpath)` names the executable rather than an
    install tree containing `bin/<tool>`; and `all_srcs` is gone.
  - Under `bzlmod` a `bazel_dep` resolves to one version graph-wide under MVS,
    so `tools.make(mode = "source", version = ...)` follows whatever is
    resolved; declare your own `bazel_dep` or `single_version_override` to move
    off the default. Every previously supported `make` and `ninja` version is
    still offered, named by the tool version rather than the registry's
    `.bcr.N` re-release suffix (`make` 4.4.1 is `make@4.4.1.bcr.1`).
  - `register_built_pkgconfig_toolchain = False` still works, but the Windows
    caveats it used to carry (needing `--enable_runfiles`, and the
    >256-character paths of the old from-source build) no longer apply.
  - `rules_foreign_cc` now requires `bazel_skylib` 1.9.2 (the `pkgconf`
    module's floor) and depends on `rules_cc_autoconf`, which `make` and
    `pkgconf` run their configure checks through. Under `WORKSPACE`,
    `rules_foreign_cc_dependencies` declares both for you.
- **`m4` is now built from its Bazel Central Registry module by default**,
  instead of being a host-`PATH`-only tool. It gained a `source` mode backed by
  `@m4//:m4` -- the same arrangement as `make`, `ninja` and `pkg-config` above
  -- and `source` is now its default mode, so a build that previously used the
  host's `m4` (or silently got no usable `m4` at all) now compiles m4 1.4.21.
  It is versioned now, so `tools.m4(version = ...)` and
  `rules_foreign_cc_dependencies(m4_version = ...)` are accepted where a
  version was previously rejected. To keep the old behavior, declare
  `tools.m4(mode = "system")` under `bzlmod`, or pass
  `register_built_tools = False` under `WORKSPACE`
  ([#1584](https://github.com/bazel-contrib/rules_foreign_cc/pull/1584)).
  - Under `WORKSPACE`, `@m4` is now a global repo name that
    `rules_foreign_cc_dependencies` claims. A consumer that declared its own
    `@m4` should rename it. Declaring it first does win -- `maybe` matches on
    the repo name alone -- but rfcc's built `m4` toolchain then points at
    whatever `@m4//:m4` resolves to in that repo, which has to be the
    executable itself and not, say, a `configure_make` install tree.
    rules_foreign_cc's own examples took the rename, to `@gnu_m4`.
- **`rules_foreign_cc` no longer registers a Python toolchain for its
  consumers.** Its `python.toolchain(python_version = "3.12")` registration is
  now a `dev_dependency`, so it applies to this repo's own builds only and
  consumers are free to pick their own interpreter. A `bzlmod` consumer that
  builds `meson` from source (which rfcc wraps in a `py_binary`) and registers
  no Python toolchain of its own now falls back to `rules_python`'s
  runtime-environment toolchain — the host interpreter — instead of the
  hermetic 3.12 rfcc used to pull in. Declare your own
  `python.toolchain(python_version = ...)` to stay hermetic
  ([#1583](https://github.com/bazel-contrib/rules_foreign_cc/pull/1583)).
- **The autogenerated `@rules_foreign_cc//toolchains:cmake_versions.bzl` data
  file was removed.** Its `CMAKE_SRCS` table moved to
  `//toolchains/private:cmake_versions.bzl` as `CMAKE_SRC_SRCS` and is no longer
  publicly loadable. It was a "DO NOT MODIFY" generated data file; out-of-tree
  loads of it (always unsupported in spirit) must drop the dependency
  ([#1567](https://github.com/bazel-contrib/rules_foreign_cc/pull/1567)).
...
- **New `noop_<tool>_toolchain` targets** under `//toolchains` (one per
  noop-capable build tool: cmake, ninja, make, meson, pkgconfig, autoconf,
  automake, m4, msbuild). nmake has no noop toolchain: it shares make's
  toolchain type, so `tools.make(mode = "noop")` no-ops the make family. Each
  is a tool-implementation target pointing the tool at a failing
  ...
  ([#1567](https://github.com/bazel-contrib/rules_foreign_cc/pull/1567)).
- **Per-tool bzlmod tag classes with mode selection.** Each tool takes a
  `mode` (some subset of `binary`, `source`, `system`, `noop`, depending on
  the tool) plus an optional pinned `version`. `mode = "noop"` selects the
  built-in `noop_<tool>_toolchain` (above), so `tools.<tool>(mode = "noop")`
  satisfies resolution without invoking the tool. Root modules can opt out of
  the rfcc-default registrations with `tools.explicit()`, or fetch a spoke
  without registering it via `register_toolchain = False`
  ([#1567](https://github.com/bazel-contrib/rules_foreign_cc/pull/1567)).
- **cmake and ninja now register a host-PATH `system` toolchain in addition
  to their prebuilt binary.** The binary toolchain still wins on the platforms
  rules_foreign_cc ships prebuilts for; on any other host, resolution now
  falls back to a `cmake`/`ninja` found on `PATH` instead of failing. This
  restores the WORKSPACE default behavior. If you relied on resolution
  *failing* on an unsupported platform, use `tools.explicit()` and declare only
  the cmake/ninja modes you want
  ([#1567](https://github.com/bazel-contrib/rules_foreign_cc/pull/1567)).
...
- Append toolchain `--sysroot`/`-isystem` flags to `CC`/`LD` (not just
  `CFLAGS`) when bootstrapping a built tool, fixing configure scripts that need
  toolchain-provided system headers
  ([#1470](https://github.com/bazel-contrib/rules_foreign_cc/pull/1470)). This
  still applies to the from-source `cmake` and `meson` builds; the `make` and
  `pkg-config` halves of it no longer run by default (see the BCR bootstrap
  change above).
- Several from-source `pkg-config` and `make` fixes landed earlier in this
  cycle and were then superseded by the move to the BCR modules, so they do not
  affect a default 0.16.0 build: a Windows build fix
  ([#1459](https://github.com/bazel-contrib/rules_foreign_cc/pull/1459)),
  compiling pkg-config with `-std=gnu90`
  ([#1458](https://github.com/bazel-contrib/rules_foreign_cc/pull/1458)), the
  faster `mirror.bazel.build` download mirror
  ([#1445](https://github.com/bazel-contrib/rules_foreign_cc/pull/1445)), and
  reproducible `make` 4.4/4.4.1 bootstrap patches
  ([#1543](https://github.com/bazel-contrib/rules_foreign_cc/pull/1543)). They
  remain relevant only if you instantiate `make_tool`/`pkgconfig_tool`
  yourself.
...
- **Bazel 9 compatibility:** load `CcSharedLibraryInfo` directly from `rules_cc`
  ...
  [#1552](https://github.com/bazel-contrib/rules_foreign_cc/pull/1552)). The
  generated `glib`/`gettext` Windows archive `BUILD` files now load `cc_import`
  explicitly, since Bazel 9 no longer autoloads it
  ([#1570](https://github.com/bazel-contrib/rules_foreign_cc/pull/1570)).
...
- Dev-only dependency bumps ahead of a Bazel 9 CI matrix (`aspect_rules_lint`
  2.x, `rules_swift`, `rules_apple`)
  ([#1571](https://github.com/bazel-contrib/rules_foreign_cc/pull/1571)), a
  `mako` bump in the examples' Python requirements plus least-privilege token
  permissions on the docs/formatting workflows
  ([#1568](https://github.com/bazel-contrib/rules_foreign_cc/pull/1568)), and
  tests made version-agnostic across Bazel 7/8/9
  ([#1572](https://github.com/bazel-contrib/rules_foreign_cc/pull/1572),
  [#1573](https://github.com/bazel-contrib/rules_foreign_cc/pull/1573)).
